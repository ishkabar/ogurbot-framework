...
public class Button
    {
        ...
        public static void PressKey2(BT7 btn)
        {
            Input inputDown = default(Input); // Zdarzenie dla naciśnięcia klawisza
            Input inputUp = default(Input);   // Zdarzenie dla zwolnienia klawisza

            // Ustawienie zdarzenia dla naciśnięcia klawisza (KEYDOWN)
            inputDown.type = 1U;
            inputDown.i_union.keyboardinput.wVk = (BT6)0;  // Nie używamy wirtualnego kodu, tylko skanera
            inputDown.i_union.keyboardinput.wScan = btn;    // Ustawiamy kod skanera (np. spacja)
            inputDown.i_union.keyboardinput.dwFlags = BT5.KEYDOWN | BT5.SCANCODE;
            inputDown.i_union.keyboardinput.time = 0;
            inputDown.i_union.keyboardinput.dwExtraInfo = (UIntPtr)0UL;

            // Ustawienie zdarzenia dla zwolnienia klawisza (KEYUP)
            inputUp.type = 1U;
            inputUp.i_union.keyboardinput.wVk = (BT6)0;    // Brak wirtualnego kodu
            inputUp.i_union.keyboardinput.wScan = btn;      // Ustawiamy ten sam kod skanera
            inputUp.i_union.keyboardinput.dwFlags = BT5.KEYUP | BT5.SCANCODE;
            inputUp.i_union.keyboardinput.time = 0;
            inputUp.i_union.keyboardinput.dwExtraInfo = (UIntPtr)0UL;

            // Najpierw wysyłamy zdarzenie naciśnięcia
            SendInput(1U, new Input[] { inputDown }, Input.Size);
            Thread.Sleep(30);
            // Następnie wysyłamy zdarzenie zwolnienia
            SendInput(1U, new Input[] { inputUp }, Input.Size);
        }
